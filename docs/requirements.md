...
| R06 | Presets, custom duration, date/time, command/process completion, unlimited | Clock, expiry, process and UI tests; long-duration device check | Signed command/PID completion, native manual stop/custom-duration expiry and user-capped unlimited trials pass; manual process/date and long-duration gates open |
...
| R10 | Homebrew installation, clean distribution and complete uninstall | Fresh install, upgrade, active-session removal and zap verification | Community helper lifecycle and local cask style/audit/install/removal pass; bundled absent-helper cleanup defect and download/upgrade/active Homebrew/zap gates open |
