...
| R09 | Swift CLI and AI skill track actual work, release all completed tasks, obey user limits | Concurrent tasks, interruptions, stale owners, PID reuse and policy-boundary tests | Signed command/PID completion, concurrency, expiry, user limits, SIGINT/SIGTERM forwarding and abrupt watcher loss verified; helper-crash/restart and remaining fault gates open |
