...
| R09 | Swift CLI and AI skill track actual work, release all completed tasks, obey user limits | Concurrent tasks, interruptions, stale owners, PID reuse and policy-boundary tests | Signed command completion, two concurrent tasks, automation refusal and continued work after helper removal verified; live expiry/PID and fault cases open |
| R10 | Homebrew installation, clean distribution and complete uninstall | Fresh install, upgrade, active-session removal and zap verification | Signed community install and inactive/active complete helper removal verified; actual Homebrew/download/upgrade/zap gates open |
